...
- JUnit 5 + Mockito (tests de Service y Controller)
...
- JDK 21 instalado
- IntelliJ IDEA
- [Laragon](https://laragon.org/) con MySQL corriendo localmente (no se usa Docker en este proyecto)
...
4. **Configurar el perfil activo** en cada Run Configuration de IntelliJ (clase `*Application`, ej. `MsPagosApplication`) agregando en "Environment variables": `SPRING_PROFILES_ACTIVE=dev`. Esto aplica a `ms_pagos`, `ms_sucursales`, `ms_empleados` y `ms_reportes` (ver sección de Perfiles). Los demás microservicios (`ms-clientes`, `ms-vehiculos`, `ms-reservas`) todavía usan un único `application.properties` sin perfiles.
5. **Orden de ejecución recomendado:**
   1. `eureka-server` (puerto 8761) — primero siempre, para que el resto se pueda registrar.
...
6. Verifica en `http://localhost:8761` (panel de Eureka) que todos los servicios aparezcan registrados.

## Perfiles de configuración

`ms_pagos`, `ms_sucursales`, `ms_empleados` y `ms_reportes` usan configuración en YAML con perfiles:

- **`application.yml`**: configuración común (nombre de la app, puerto). No fija un perfil activo por defecto — se elige explícitamente al ejecutar.
- **`application-dev.yml`**: perfil de desarrollo. Datasource MySQL real apuntando a Laragon (`localhost:3306`), `ddl-auto: update`, `show-sql: true`, registro en Eureka.
- **`application-test.yml`**: perfil usado por los tests automatizados (`@DataJpaTest`, etc.). Base de datos H2 en memoria, `ddl-auto: create-drop`, sin Eureka, para que los tests corran de forma aislada sin depender de MySQL ni del resto de los servicios.

Para correr en modo desarrollo: `SPRING_PROFILES_ACTIVE=dev` (variable de entorno o `-Dspring.profiles.active=dev` como VM option en IntelliJ).
...
- `ms-pagos`: `http://localhost:8084/doc/swagger-ui.html` (ruta personalizada)
...
1. Con todos los servicios arriba (ver orden de ejecución), abre el Swagger UI del microservicio que quieras probar (o el del Gateway si prefieres ir por ahí) y ejecuta los endpoints directamente desde ahí ("Try it out").
2. Alternativamente, usa Postman, Insomnia o `curl` contra:
...
3. Las respuestas de listado y detalle incluyen enlaces HATEOAS (`_links`, `_embedded`) generados por los `RepresentationModelAssembler` de cada entidad.
4. Los errores (404, 400, 500) devuelven un cuerpo JSON consistente con `timestamp`, `status`, `error`, `mensaje` y `path`, generado por el `GlobalExceptionHandler` de cada servicio.
5. Cada uno de `ms_pagos`, `ms_sucursales`, `ms_empleados` y `ms_reportes` carga automáticamente al arrancar (vía `CommandLineRunner`) al menos 5 registros de ejemplo por entidad, si la base de datos está vacía — así se puede probar sin cargar datos a mano.
